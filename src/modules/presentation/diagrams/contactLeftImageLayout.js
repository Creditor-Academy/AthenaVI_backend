/**
 * Contact Left Image
 * Layout ID: contact_left_image_v1
 *
 * Full-bleed photo on the left, overlapping contact card on the right.
 * Right / bottom twins keep their own look.
 */

function isContactLeftImageLayout(layoutId, schema) {
  const id = String(layoutId || schema?.layout_id || schema?.variant || '').toLowerCase()
  if (/right|bottom/i.test(id)) return false
  return id === 'contact_left_image_v1' || id === 'contact_left_image'
}

const CONTACT_LEFT_IMAGE_DEFAULTS = {
  HEADING: 'Contact me',
...
function buildContactLeftImageChromeSvg() {
  const {
  ...
  return `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1920 1080" width="100%" height="100%" preserveAspectRatio="none">
    <defs>
      <filter id="cliShadow" x="-8%" y="-6%" width="120%" height="120%">
        <feDropShadow dx="0" dy="18" stdDeviation="22" flood-color="#94A3B8" flood-opacity="0.22" />
      </filter>
    </defs>
    <ellipse cx="1760" cy="80" rx="280" ry="160" fill="currentColor" opacity="0.08"/>
    <ellipse cx="1680" cy="1040" rx="240" ry="140" fill="currentColor" opacity="0.06"/>
    <rect x="${panelX}" y="${panelY}" width="${panelW}" height="${panelH}" rx="${panelR}" fill="#FFFFFF" filter="url(#cliShadow)"/>
    <rect x="${accentX}" y="${accentY}" width="${accentW}" height="${accentH}" rx="4" fill="currentColor"/>
    ${iconMarkup}
    <rect x="888" y="980" width="220" height="6" rx="3" fill="currentColor" opacity="0.85"/>
    <rect x="1120" y="980" width="72" height="6" rx="3" fill="currentColor" opacity="0.35"/>
  </svg>`
}
...
function buildElements({
  ...
  const d = CONTACT_LEFT_IMAGE_DEFAULTS
  const rows = [
  ...
  return [
    {
      ...
      content: {
        svg: buildContactLeftImageChromeSvg(),
...
function layoutContactLeftImage(docOrElements, schema = {}, palette = {}, canvas = {}) {
  ...
  const out = buildElements({
    canvasW,
    canvasH,
    heading: textOf(prev.HEADING, CONTACT_LEFT_IMAGE_DEFAULTS.HEADING),
    addressLabel: textOf(prev.CONTACT_ADDRESS_LABEL, CONTACT_LEFT_IMAGE_DEFAULTS.CONTACT_ADDRESS_LABEL),
    address: textOf(prev.CONTACT_ADDRESS, CONTACT_LEFT_IMAGE_DEFAULTS.CONTACT_ADDRESS),
    phoneLabel: textOf(prev.CONTACT_PHONE_LABEL, CONTACT_LEFT_IMAGE_DEFAULTS.CONTACT_PHONE_LABEL),
    phone: textOf(prev.CONTACT_PHONE, CONTACT_LEFT_IMAGE_DEFAULTS.CONTACT_PHONE),
    emailLabel: textOf(prev.CONTACT_EMAIL_LABEL, CONTACT_LEFT_IMAGE_DEFAULTS.CONTACT_EMAIL_LABEL),
    email: textOf(prev.CONTACT_EMAIL, CONTACT_LEFT_IMAGE_DEFAULTS.CONTACT_EMAIL),
...
function buildContactLeftImageCanvasElements({ schema, options = {} } = {}) {
...
function contactLeftImagePreviewSvg() {
  const chrome = buildContactLeftImageChromeSvg()
    .replace(/currentColor/g, '#6366F1')
    .replace(/^<svg[^>]*>/, '')
    .replace(/<\/svg>\s*$/, '')
  const { headingX, headingY, headingH, labelX, rowYs, valueYOff } = GEOM
  const rows = [
  ...
  return `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1920 1080" width="100%" height="100%">
    ...
    <defs>
      ...
      <linearGradient id="cliHill" x1="0" y1="0" x2="0" y2="1">
        <stop offset="0%" stop-color="#8FBF8E"/>
        <stop offset="100%" stop-color="#5E945D"/>
      </linearGradient>
    </defs>
    <rect width="920" height="1080" fill="url(#cliSky)"/>
    <ellipse cx="220" cy="180" rx="110" ry="48" fill="#FFFFFF" opacity="0.95"/>
    <ellipse cx="320" cy="180" rx="80" ry="36" fill="#FFFFFF" opacity="0.95"/>
    <path d="M0 520 C180 400 360 480 540 430 C700 390 780 500 920 440 L920 1080 L0 1080 Z" fill="url(#cliHill)"/>
    ${chrome}
...
module.exports = {
  ...
  CONTACT_LEFT_IMAGE_DEFAULTS,
  buildContactLeftImageChromeSvg,
