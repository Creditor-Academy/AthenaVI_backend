/**
 * Speaker Bio Image Right
 * Layout ID: speaker_bio_image_right_v1
 *
 * Name / role / bio on a left card, portrait on the right.
 * Left and centered twins keep their own engines.
 */

function isSpeakerBioImageRightLayout(layoutId, schema) {
  const id = String(layoutId || schema?.layout_id || schema?.variant || '').toLowerCase()
  if (/centered|team_speaker/i.test(id)) return false
  return id === 'speaker_bio_image_right_v1' || id === 'speaker_bio_image_right'
}

const SPEAKER_BIO_IMAGE_RIGHT_DEFAULTS = {
...
function buildSpeakerBioImageRightChromeSvg() {
  const { panelX, panelY, panelW, panelH, panelR, accentX, accentY, accentW, accentH } = GEOM
  return `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1920 1080" width="100%" height="100%" preserveAspectRatio="none">
    <defs>
      <filter id="sbirShadow" x="-8%" y="-8%" width="120%" height="120%">
        <feDropShadow dx="0" dy="16" stdDeviation="20" flood-color="#94A3B8" flood-opacity="0.2" />
      </filter>
    </defs>
    <ellipse cx="160" cy="70" rx="260" ry="140" fill="currentColor" opacity="0.08"/>
    <ellipse cx="240" cy="1040" rx="220" ry="120" fill="currentColor" opacity="0.06"/>
    <rect x="${panelX}" y="${panelY}" width="${panelW}" height="${panelH}" rx="${panelR}" fill="#FFFFFF" filter="url(#sbirShadow)"/>
...
function buildElements({ canvasW, canvasH, name, role, bio, imageUrl, accent, textColor, mutedColor, prev = {} }) {
  ...
  const d = SPEAKER_BIO_IMAGE_RIGHT_DEFAULTS
  return [
    {
      ...
      content: {
        svg: buildSpeakerBioImageRightChromeSvg(),
...
function layoutSpeakerBioImageRight(docOrElements, schema = {}, palette = {}, canvas = {}) {
  ...
  const out = buildElements({
    canvasW,
    canvasH,
    name: textOf(prev.MEMBER_1_NAME, SPEAKER_BIO_IMAGE_RIGHT_DEFAULTS.MEMBER_1_NAME),
    role: textOf(prev.MEMBER_1_ROLE, SPEAKER_BIO_IMAGE_RIGHT_DEFAULTS.MEMBER_1_ROLE),
    bio: textOf(prev.MEMBER_1_BIO, SPEAKER_BIO_IMAGE_RIGHT_DEFAULTS.MEMBER_1_BIO),
...
function buildSpeakerBioImageRightCanvasElements({ schema, options = {} } = {}) {
...
function speakerBioImageRightPreviewSvg() {
  const chrome = buildSpeakerBioImageRightChromeSvg()
  ...
  return `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1920 1080" width="100%" height="100%">
    ...
    <defs>
      <clipPath id="sbirClip"><rect x="${imgX}" y="${imgY}" width="${imgW}" height="${imgH}" rx="${radius}"/></clipPath>
      <linearGradient id="sbirSky" x1="0" y1="0" x2="0" y2="1">
        <stop offset="0%" stop-color="#7EC8E8"/>
        <stop offset="55%" stop-color="#B7D4E8"/>
        <stop offset="100%" stop-color="#D0E3EF"/>
      </linearGradient>
      <linearGradient id="sbirHill" x1="0" y1="0" x2="0" y2="1">
        <stop offset="0%" stop-color="#8FBF8E"/>
        <stop offset="100%" stop-color="#5E945D"/>
      </linearGradient>
    </defs>
    <g clip-path="url(#sbirClip)">
      <rect x="${imgX}" y="${imgY}" width="${imgW}" height="${imgH}" fill="url(#sbirSky)"/>
      ...
      <path d="M${imgX} ${imgY + 420} C${imgX + 180} ${imgY + 300} ${imgX + 380} ${imgY + 380} ${imgX + 560} ${imgY + 320} C${imgX + 680} ${imgY + 280} ${imgX + 720} ${imgY + 400} ${imgX + imgW} ${imgY + 340} L${imgX + imgW} ${imgY + imgH} L${imgX} ${imgY + imgH} Z" fill="url(#sbirHill)"/>
    </g>
    ${chrome}
...
module.exports = {
  ...
  SPEAKER_BIO_IMAGE_RIGHT_DEFAULTS,
  buildSpeakerBioImageRightChromeSvg,
