/**
 * Chart three cards — three side-by-side metric cards with icons, charts, and insights.
 * Layout id: chart_three_cards_v1.
 */
...
function isChartThreeCardsLayout(layoutId) {
  return /chart_three_cards_v1$/i.test(String(layoutId || ''))
}

function isChartThreeCardsTextSlot(slotId) {
...
function insightIconCircleSvg(color) {
  const size = CTC3_GEOM.insightIconSize
  return '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 ' + size + ' ' + size + '" width="100%" height="100%" preserveAspectRatio="xMidYMid meet">' +
...
function chartThreeCardsChromeSpecs() {
  const g = CTC3_GEOM
  const card1X = 15
  ...
  // Card 1
  const card1Specs = [
    { slotId: 'CTC3_CARD_1_BG', x: card1X, y: g.cardStartY, w: g.cardW, h: g.cardH, color: '#FFFFFF', layer: 3, kind: 'cardBg' },
    { slotId: 'CTC3_CARD_1_ICON_CIRCLE', x: card1X + g.iconX, y: g.cardStartY + g.iconY, w: g.iconSize, h: g.iconSize, color: CTC3_COLORS.card1, layer: 4, kind: 'iconCircle' },
    { slotId: 'CTC3_CARD_1_ICON', x: card1X + g.iconX, y: g.cardStartY + g.iconY, w: g.iconSize, h: g.iconSize, color: CTC3_COLORS.card1, layer: 5, kind: 'barChartIcon' },
    { slotId: 'CTC3_CARD_1_INSIGHT_BG', x: card1X + g.insightX, y: g.cardStartY + g.insightY, w: g.insightW, h: g.insightH, color: CTC3_COLORS.card1, layer: 4, kind: 'insightBox' },
    { slotId: 'CTC3_CARD_1_INSIGHT_ICON_CIRCLE', x: card1X + g.insightX + 8, y: g.cardStartY + g.insightY + 10, w: g.insightIconSize, h: g.insightIconSize, color: CTC3_COLORS.card1, layer: 5, kind: 'insightIconCircle' },
    { slotId: 'CTC3_CARD_1_INSIGHT_ICON', x: card1X + g.insightX + 8, y: g.cardStartY + g.insightY + 10, w: g.insightIconSize, h: g.insightIconSize, color: CTC3_COLORS.card1, layer: 6, kind: 'trendUpIcon' },
  ]
  
  // Card 2
  const card2Specs = [
    { slotId: 'CTC3_CARD_2_BG', x: card2X, y: g.cardStartY, w: g.cardW, h: g.cardH, color: '#FFFFFF', layer: 3, kind: 'cardBg' },
    { slotId: 'CTC3_CARD_2_ICON_CIRCLE', x: card2X + g.iconX, y: g.cardStartY + g.iconY, w: g.iconSize, h: g.iconSize, color: CTC3_COLORS.card2, layer: 4, kind: 'iconCircle' },
    { slotId: 'CTC3_CARD_2_ICON', x: card2X + g.iconX, y: g.cardStartY + g.iconY, w: g.iconSize, h: g.iconSize, color: CTC3_COLORS.card2, layer: 5, kind: 'targetIcon' },
    { slotId: 'CTC3_CARD_2_INSIGHT_BG', x: card2X + g.insightX, y: g.cardStartY + g.insightY, w: g.insightW, h: g.insightH, color: CTC3_COLORS.card2, layer: 4, kind: 'insightBox' },
    { slotId: 'CTC3_CARD_2_INSIGHT_ICON_CIRCLE', x: card2X + g.insightX + 8, y: g.cardStartY + g.insightY + 10, w: g.insightIconSize, h: g.insightIconSize, color: CTC3_COLORS.card2, layer: 5, kind: 'insightIconCircle' },
    { slotId: 'CTC3_CARD_2_INSIGHT_ICON', x: card2X + g.insightX + 8, y: g.cardStartY + g.insightY + 10, w: g.insightIconSize, h: g.insightIconSize, color: CTC3_COLORS.card2, layer: 6, kind: 'chartLineIcon' },
  ]
  
  // Card 3
  const card3Specs = [
    { slotId: 'CTC3_CARD_3_BG', x: card3X, y: g.cardStartY, w: g.cardW, h: g.cardH, color: '#FFFFFF', layer: 3, kind: 'cardBg' },
    { slotId: 'CTC3_CARD_3_ICON_CIRCLE', x: card3X + g.iconX, y: g.cardStartY + g.iconY, w: g.iconSize, h: g.iconSize, color: CTC3_COLORS.card3, layer: 4, kind: 'iconCircle' },
    { slotId: 'CTC3_CARD_3_ICON', x: card3X + g.iconX, y: g.cardStartY + g.iconY, w: g.iconSize, h: g.iconSize, color: CTC3_COLORS.card3, layer: 5, kind: 'peopleIcon' },
    { slotId: 'CTC3_CARD_3_INSIGHT_BG', x: card3X + g.insightX, y: g.cardStartY + g.insightY, w: g.insightW, h: g.insightH, color: CTC3_COLORS.card3, layer: 4, kind: 'insightBox' },
    { slotId: 'CTC3_CARD_3_INSIGHT_ICON_CIRCLE', x: card3X + g.insightX + 8, y: g.cardStartY + g.insightY + 10, w: g.insightIconSize, h: g.insightIconSize, color: CTC3_COLORS.card3, layer: 5, kind: 'insightIconCircle' },
    { slotId: 'CTC3_CARD_3_INSIGHT_ICON', x: card3X + g.insightX + 8, y: g.cardStartY + g.insightY + 10, w: g.insightIconSize, h: g.insightIconSize, color: CTC3_COLORS.card3, layer: 6, kind: 'checkIcon' },
  ]
  
  return specs.concat(card1Specs).concat(card2Specs).concat(card3Specs)
}

function chartThreeCardsOverlay(gx, gy, gw, gh) {
  const g = CTC3_GEOM
  const sx = gw / g.viewW
  const sy = gh / g.viewH
...
function specToChartThreeCardsContent(spec) {
...
function layoutChartThreeCards(elements, schema, palette, canvas) {
  if (!Array.isArray(elements)) return elements
  palette = palette || {}
  canvas = canvas || {}
  const canvasW = canvas.width || 1920
  const canvasH = canvas.height || 1080
  const sx = canvasW / CTC3_GEOM.viewW
  const sy = canvasH / CTC3_GEOM.viewH
  const overlay = chartThreeCardsOverlay(0, 0, canvasW, canvasH)
  const chromeRe = /^CTC3_/i
  ...
  const filtered = elements.filter(function(el) {
    return !chromeRe.test(String(el.slotId || '')) && isChartThreeCardsTextSlot(el.slotId)
  })
  ...
  function placeText(slotId, box, style, role) {
    ...
    return {
      id: (prev && prev.id) || newId('txt-ctc3'),
      type: 'text',
      slotId: slotId,
  ...
  const chrome = chartThreeCardsChromeSpecs().map(function(spec) {
    const prev = prevBySlot.get(spec.slotId.toUpperCase())
    const graphic = specToChartThreeCardsContent(spec)
    if (!graphic) return null
    return {
      id: (prev && prev.id) || newId('shp-ctc3'),
      type: 'graphic',
      layer: spec.layer || 4,
      placement: {
...
module.exports = {
  isChartThreeCardsLayout: isChartThreeCardsLayout,
  layoutChartThreeCards: layoutChartThreeCards,
  CTC3_GEOM: CTC3_GEOM,
  CTC3_DEFAULTS: CTC3_DEFAULTS,
}
