function isParaTitleLeftImageBoxedLayout(layoutId, schema) {
  const id = String(layoutId || schema?.layout_id || schema?.variant || '').toLowerCase();
  return id === 'para_title_left_image_boxed_v1' || id === 'para_title_left_image_boxed';
}

const PARA_TITLE_LEFT_IMAGE_BOXED_DEFAULTS = {
...
function buildParaTitleLeftImageBoxedChromeSvg() {
  ...
  return `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1920 1080" width="100%" height="100%" preserveAspectRatio="none">
    <defs>
      <filter id="ptlibShadow" x="-12%" y="-10%" width="124%" height="128%">
        <feDropShadow dx="0" dy="18" stdDeviation="22" flood-color="#94A3B8" flood-opacity="0.22" />
      </filter>
    </defs>
    <rect x="${frameX}" y="${frameY}" width="${frameW}" height="${frameH}" rx="${frameR}" fill="#FFFFFF" filter="url(#ptlibShadow)" />
  ...
  return [
    {
      ...
      content: {
        svg: buildParaTitleLeftImageBoxedChromeSvg(),
...
function layoutParaTitleLeftImageBoxed(docOrElements, schema = {}, palette = {}, canvas = {}) {
  ...
  const out = buildElements({
    canvasW,
    canvasH,
    headingText: textOf(headingEl, PARA_TITLE_LEFT_IMAGE_BOXED_DEFAULTS.HEADING),
    bodyText: textOf(bodyEl, PARA_TITLE_LEFT_IMAGE_BOXED_DEFAULTS.BODY),
