function isParaTitleLeftImageOverlayLayout(layoutId, schema) {
  const id = String(layoutId || schema?.layout_id || schema?.variant || '').toLowerCase();
  return id === 'para_title_left_image_overlay_v1' || id === 'para_title_left_image_overlay';
}

const PARA_TITLE_LEFT_IMAGE_OVERLAY_DEFAULTS = {
...
function buildParaTitleLeftImageOverlaySvg() {
  const { textX, barY, barW, barH } = GEOM;
  return `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1920 1080" width="100%" height="100%" preserveAspectRatio="none">
    <defs>
      <linearGradient id="ptlioFade" x1="0" y1="0" x2="1" y2="0">
    ...
    <rect width="1920" height="1080" fill="url(#ptlioFade)" />
...
function buildElements({ canvasW, canvasH, headingText, bodyText, imageUrl, accent, prev = {} }) {
  ...
  return [
    ...
    },
    {
      ...
      placement: { x: 0, y: 0, width: canvasW, height: canvasH, rotation: 0, opacity: 1 },
      content: {
        svg: buildParaTitleLeftImageOverlaySvg(),
...
function layoutParaTitleLeftImageOverlay(docOrElements, schema = {}, palette = {}, canvas = {}) {
  ...
  const out = buildElements({
    canvasW,
    canvasH,
    headingText: textOf(headingEl, PARA_TITLE_LEFT_IMAGE_OVERLAY_DEFAULTS.HEADING),
    bodyText: textOf(bodyEl, PARA_TITLE_LEFT_IMAGE_OVERLAY_DEFAULTS.BODY),
