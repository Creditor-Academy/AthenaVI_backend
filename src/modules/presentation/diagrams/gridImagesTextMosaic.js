/**
 * Grid images text mosaic — Asymmetric masonry layout with one large card (A) and two smaller stacked cards (B, C).
 * Layout id: grid_images_text_mosaic_v1.
 */
...
const GITM_GEOM = {
  ...
  // Three cards side-by-side (horizontal)
  card1X: 30,
  card1Y: 40,
  card1W: 300,
  card1H: 480,
  ...
  // Inside each card (relative to card top)
  imageY: 20,
  imageH: 180,
  imageRadius: 12,
  
  // Text positions (from card top)
  titleY: 220,
  titleH: 32,
  bodyY: 262,
  bodyH: 198,
...
const GITM_DEFAULTS = {
  ...
  COL_2_BODY: 'Supporting paragraph with three to four lines of scannable copy that explains the key idea without overwhelming the slide.',
  
  COL_3_TITLE: 'Feature C',
  COL_3_BODY: 'Supporting paragraph with three to four lines of scannable copy that explains the key idea without overwhelming the slide.',
}

const isGridImagesTextMosaicLayout = (layoutId) => {
  return /grid_images_text_mosaic_v1$/i.test(String(layoutId || ''))
}

const isGridImagesTextMosaicSlot = (slotId) => {
...
const cardSvg = (w, h, radius) => {
  return `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 ${w} ${h}" width="100%" height="100%" preserveAspectRatio="none">
    <defs>
      <filter id="cardShadowMosaic" x="-50%" y="-50%" width="200%" height="200%">
    ...
    <rect x="0" y="0" width="${w}" height="${h}" fill="${GITM_COLORS.card}" stroke="${GITM_COLORS.cardBorder}" stroke-width="1" rx="${radius}" filter="url(#cardShadowMosaic)"/>
  </svg>`
}
...
const imagePlaceholderSvg = (w, h) => {
  return `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 ${w} ${h}" width="100%" height="100%" preserveAspectRatio="none">
    <defs>
      <linearGradient id="imgGradMosaic" x1="0%" y1="0%" x2="100%" y2="100%">
        <stop offset="0%" style="stop-color:#E0F2FE;stop-opacity:1" />
        <stop offset="100%" style="stop-color:#BAE6FD;stop-opacity:1" />
      </linearGradient>
    </defs>
    <rect x="0" y="0" width="${w}" height="${h}" fill="url(#imgGradMosaic)" rx="12"/>
    <!-- Clouds -->
...
const gridImagesTextMosaicChromeSpecs = () => {
  const g = GITM_GEOM
  const specs = []
  ...
  cardXs.forEach((x, i) => {
    specs.push({
      slotId: `GITM_CARD${i+1}_BG`,
      x: x,
      y: g.card1Y,
      w: g.card1W,
      h: g.card1H,
      color: GITM_COLORS.card,
      layer: 2,
      kind: 'card',
    })
  })
...
const gridImagesTextMosaicOverlay = (gx, gy, gw, gh) => {
  const g = GITM_GEOM
  const sx = gw / g.viewW
  const sy = gh / g.viewH
  const box = (x, y, w, h) => ({
  ...
  const innerW = g.card1W - (g.cardPadding * 2)
  const cardXs = [g.card1X, g.card2X, g.card3X]
  ...
  cardXs.forEach((cardX, i) => {
    const num = i + 1
    overlays[`title${num}`] = box(cardX + g.cardPadding, g.card1Y + g.titleY, innerW, g.titleH)
    overlays[`body${num}`] = box(cardX + g.cardPadding, g.card1Y + g.bodyY, innerW, g.bodyH)
  })
  
  return overlays
}

const specToGridImagesTextMosaicContent = (spec) => {
  if (spec.kind === 'card') {
    return {
      svg: cardSvg(spec.w, spec.h, GITM_GEOM.cardRadius),
      colorMode: 'fixed',
      fill: spec.color,
    }
  }
...
const layoutGridImagesTextMosaic = (elements, schema, palette = {}, canvas = {}) => {
  if (!Array.isArray(elements)) return elements
  const canvasW = canvas.width || 1920
  const canvasH = canvas.height || 1080
  const sx = canvasW / GITM_GEOM.viewW
  const sy = canvasH / GITM_GEOM.viewH
  const overlay = gridImagesTextMosaicOverlay(0, 0, canvasW, canvasH)
  const chromeRe = /^GITM_/i
  
  const prevBySlot = new Map(
    elements.filter((el) => chromeRe.test(String(el.slotId || ''))).map((el) => [String(el.slotId || '').toUpperCase(), el])
  )
  
  const filtered = elements.filter((el) => !chromeRe.test(String(el.slotId || '')) && isGridImagesTextMosaicSlot(el.slotId))
  ...
  cardXs.forEach((cardX, i) => {
    ...
    // Image
    next.push(placeImage(`COL_${num}_IMAGE`, cardX, g.card1Y, innerW, g.imageH, g.imageY))
  ...
  const chrome = gridImagesTextMosaicChromeSpecs().map((spec) => {
    const prev = prevBySlot.get(spec.slotId.toUpperCase())
    const graphic = specToGridImagesTextMosaicContent(spec)
    if (!graphic) return null
    return {
      id: prev?.id || newId('shp-gitm'),
      type: 'graphic',
      layer: spec.layer || 2,
      placement: {
