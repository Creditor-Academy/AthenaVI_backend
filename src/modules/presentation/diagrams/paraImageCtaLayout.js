/**
 * Para Image CTA
 * Layout ID: para_image_cta_v1
 *
 * Body + Book a demo pill on the left, tall rounded photo on the right.
 * Image Para CTA and Overlay Image CTA keep their own look.
 */

function isParaImageCtaLayout(layoutId, schema) {
  const id = String(layoutId || schema?.layout_id || schema?.variant || '').toLowerCase()
  if (/image_para|overlay/i.test(id)) return false
  return id === 'para_image_cta_v1' || id === 'para_image_cta'
}

const PARA_IMAGE_CTA_DEFAULTS = {
...
function buildParaImageCtaChromeSvg() {
  ...
  return `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1920 1080" width="100%" height="100%" preserveAspectRatio="none">
    <defs>
      <filter id="pictaShadow" x="-10%" y="-8%" width="122%" height="124%">
        <feDropShadow dx="0" dy="16" stdDeviation="20" flood-color="#94A3B8" flood-opacity="0.2" />
      </filter>
    </defs>
    <ellipse cx="160" cy="80" rx="220" ry="120" fill="currentColor" opacity="0.08"/>
    <rect x="${barX}" y="${barY}" width="${barW}" height="${barH}" rx="${barH / 2}" fill="currentColor"/>
    <rect x="${frameX}" y="${frameY}" width="${frameW}" height="${frameH}" rx="${frameR}" fill="#FFFFFF" filter="url(#pictaShadow)"/>
  </svg>`
}

function buildParaImageCtaPillSvg() {
...
function buildElements({ canvasW, canvasH, body, cta, imageUrl, accent, mutedColor, prev = {} }) {
  ...
  const d = PARA_IMAGE_CTA_DEFAULTS
  ...
  return [
    {
      ...
      content: {
        svg: buildParaImageCtaChromeSvg(),
...
function layoutParaImageCta(docOrElements, schema = {}, palette = {}, canvas = {}) {
  ...
  const out = buildElements({
    canvasW,
    canvasH,
    body: textOf(prev.BODY, PARA_IMAGE_CTA_DEFAULTS.BODY),
    cta: textOf(prev.CTA, PARA_IMAGE_CTA_DEFAULTS.CTA),
...
function buildParaImageCtaCanvasElements({ schema, options = {} } = {}) {
...
function paraImageCtaPreviewSvg() {
  const chrome = buildParaImageCtaChromeSvg()
  ...
  return `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1920 1080" width="100%" height="100%">
    ...
    <defs>
      <clipPath id="pictaClip"><rect x="${imgX}" y="${imgY}" width="${imgW}" height="${imgH}" rx="${radius}"/></clipPath>
      <linearGradient id="pictaSky" x1="0" y1="0" x2="0" y2="1">
        <stop offset="0%" stop-color="#7EC8E8"/>
        <stop offset="55%" stop-color="#B7D4E8"/>
        <stop offset="100%" stop-color="#D0E3EF"/>
      </linearGradient>
      <linearGradient id="pictaHill" x1="0" y1="0" x2="0" y2="1">
        <stop offset="0%" stop-color="#8FBF8E"/>
        <stop offset="100%" stop-color="#5E945D"/>
      </linearGradient>
    </defs>
    ${chrome}
    <g clip-path="url(#pictaClip)">
      <rect x="${imgX}" y="${imgY}" width="${imgW}" height="${imgH}" fill="url(#pictaSky)"/>
      ...
      <path d="M${imgX} ${imgY + 520} C${imgX + 180} ${imgY + 400} ${imgX + 360} ${imgY + 480} ${imgX + 520} ${imgY + 450} C${imgX + 680} ${imgY + 420} ${imgX + 760} ${imgY + 500} ${imgX + imgW} ${imgY + 430} L${imgX + imgW} ${imgY + imgH} L${imgX} ${imgY + imgH} Z" fill="url(#pictaHill)"/>
    </g>
    ${bodyMarkup}
...
module.exports = {
  ...
  PARA_IMAGE_CTA_DEFAULTS,
  buildParaImageCtaChromeSvg,
  buildParaImageCtaPillSvg,
  layoutParaImageCta,
