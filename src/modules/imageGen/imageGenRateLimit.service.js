...
const GENERATE_WINDOW_SEC =
  Number(process.env.IMAGE_GEN_RATE_LIMIT_WINDOW_SEC) > 0
    ? Number(process.env.IMAGE_GEN_RATE_LIMIT_WINDOW_SEC)
    : 3600;

const GENERATE_MAX =
  Number(process.env.IMAGE_GEN_RATE_LIMIT_MAX) > 0
    ? Number(process.env.IMAGE_GEN_RATE_LIMIT_MAX)
    : 30;

const REGENERATE_WINDOW_SEC =
  Number(process.env.IMAGE_GEN_REGENERATE_RATE_LIMIT_WINDOW_SEC) > 0
    ? Number(process.env.IMAGE_GEN_REGENERATE_RATE_LIMIT_WINDOW_SEC)
    : 3600;

const REGENERATE_MAX =
  Number(process.env.IMAGE_GEN_REGENERATE_RATE_LIMIT_MAX) > 0
    ? Number(process.env.IMAGE_GEN_REGENERATE_RATE_LIMIT_MAX)
    : 60;

const userGenerateKey = (userId) => `imageGen:generate:user:${userId || 'unknown'}`;
const workspaceGenerateKey = (workspaceId) =>
  `imageGen:generate:workspace:${workspaceId || 'unknown'}`;
const userRegenerateKey = (userId) => `imageGen:regenerate:user:${userId || 'unknown'}`;
const workspaceRegenerateKey = (workspaceId) =>
  `imageGen:regenerate:workspace:${workspaceId || 'unknown'}`;
...
async function assertBucketsAllowed({ userKey, workspaceKey, max, windowSec }) {
  ...
  if (userState.blocked || workspaceState.blocked) {
    const err = new AppError(messages.IMAGE_GEN_RATE_LIMITED, 429);
