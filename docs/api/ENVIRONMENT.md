...
- **Storage upgrade requests** – `PLATFORM_SUPERADMIN_NOTIFICATION_EMAIL` (comma-separated inbox for upgrade-request emails; falls back to `PLATFORM_SUPERADMIN_EMAILS` if unset). Optional `STORAGE_UPGRADE_REQUEST_COOLDOWN_SEC` (default **86400** = 24h per user).
